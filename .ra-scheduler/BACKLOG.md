# RA KB 자동보강 BACKLOG (우선순위 큐)

> 매 회차 스케줄 러닝 시작 시 최상단 미완료 항목 선택.
> 완료 항목은 `[x]` + `done-YYYY-MM-DD` 태그 처리 후 유지 (이력 추적).
> 큰 주제는 세분화하여 하위 항목으로 재등록.
> 최초 작성: 2026-04-22

## EP(Effort Point) 기준
| 등급 | EP | 기준 |
|---|---|---|
| `[EP:1]` S | 1 | 체크리스트·목록·단순 비교표. 리서치 범위 단일 지역 or 단일 표준 |
| `[EP:2]` M | 2 | 단일 지역 심화 가이드 또는 단일 제품 템플릿 |
| `[EP:3]` L | 3 | 3개 지역 비교 + 3개 제품 맵핑 동반 대형 문서 |

**회차 예산**: 1회차(03:00) 3 EP / 2회차(19:30) 3 EP / CATCHUP 회차 4 EP
**처리 원칙**: 예산 소진 시 중단 → carryover. 예산 내 복수 처리 가능.

---

## [P1] 고우선 — 기초 골격 완성

### 1. 규제 지역별 심화
- [x] FDA eSTAR 섹션별 작성 가이드: Device Description / Indications for Use `[EP:3]` `done-2026-04-22 (#2 push)`
- [x] FDA eSTAR 섹션별 작성 가이드: Substantial Equivalence 비교표 작성법 `[EP:3]` `done-2026-04-22 (#2 push)`
- [x] FDA eSTAR 섹션별 작성 가이드: Performance Testing / Bench Test `[EP:3]` `done-2026-04-24 (#3)`
- [x] FDA eSTAR 섹션별 작성 가이드: Cybersecurity Section (524B 기반) `[EP:3]` `done-2026-04-29 (#4)`
- [x] FDA eSTAR 섹션별 작성 가이드: Software Section (62304 기반) `[EP:3]` `done-2026-04-30 (#5)`
- [x] FDA eSTAR 섹션별 작성 가이드: Labeling (IFU, Form 3881) `[EP:2]` `done-2026-04-30 (#6)`
- [x] FDA Pre-Submission (Q-Sub) 작성 가이드·샘플 질문 `[EP:2]` `done-2026-05-01 (#7)`
- [x] MFDS 기술문서 섹션별 작성 가이드 (사용목적·작용원리·성능·안전성) `[EP:3]` `done-2026-05-01 (#8)`
- [x] MFDS 디지털의료제품법 하위 고시 추적 리스트 `[EP:1]` `done-2026-05-01 (#9)`
- [x] MFDS 디지털의료기기 6종 가이드라인 요약 (2025-05-07) `[EP:2]` `done-2026-05-02 (#10)`
- [x] MDR Annex II Technical Documentation 섹션별 템플릿 `[EP:3]` `done-2026-05-02 (#11)`
- [x] MDR Annex III PMS TD 템플릿 `[EP:2]` `done-2026-05-03 (#12)`
- [x] MDR Annex I GSPR 체크리스트 (전항 맵핑) `[EP:2]` `done-2026-05-03 (#12)`
- [x] MDR Declaration of Conformity (DoC) 양식·작성 지침 `[EP:1]` `done-2026-05-03 (#13)`
- [x] MDR Article 15 PRRC 자격 요건·역할 정리 `[EP:1]` `done-2026-05-03 (#13)`

### 2. 제품별 기술파일 뼈대
- [x] X-ray Detector DHF 목차·각 문서 템플릿 `[EP:2]` `done-2026-05-03 (#13)`
- [x] X-ray Detector 성능 시험 항목 매트릭스 (IEC 62220-1-1 DQE 등) `[EP:2]` `done-2026-05-04 (#14)`
- [x] Handheld X-ray Source DHF 목차·각 문서 템플릿 `[EP:2]` `done-2026-05-04 (#15)`
- [x] Handheld X-ray Source 21 CFR 1020.30 적합성 체크리스트 `[EP:1]` `done-2026-05-04 (#16)`
- [x] 촬영실 GUI SW 문서 세트 (IEC 62304: SRS, SDS, SUP, SVP, 설계 등) `[EP:3]` `done-2026-05-05 (#17)`
- [x] 3개 제품 공통 Risk Management File (ISO 14971:2019) 템플릿 `[EP:3]` `done-2026-05-05 (#18)`
- [x] 3개 제품 공통 Usability Engineering File (IEC 62366-1:2015+AMD1:2020) 템플릿 `[EP:2]` `done-2026-05-06 (#19)`
- [x] 3개 제품 공통 Clinical Evaluation Plan/Report (MDR) 템플릿 `[EP:2]` `done-2026-05-06 (#20)`
- [x] 3개 제품 공통 Biological Evaluation (ISO 10993-1) 적용 판단 `[EP:2]` `done-2026-05-07 (#21)`

### 3. 시험·표준 실무
- [x] IEC 60601-1:2005+A1+A2 (Ed 3.2) 시험 항목 매트릭스 `[EP:3]` `done-2026-05-07 (#23)`
- [x] IEC 60601-1-2:2014+A1:2020 EMC 시험 항목·Pass Criteria `[EP:2]` `done-2026-05-08 (#24)`
- [x] IEC 60601-1-3:2008+A1+A2 방사선 방호 시험 항목 `[EP:2]` `done-2026-05-08 (#25)`
- [x] IEC 60601-2-28:2017 X-ray Source 시험 항목 `[EP:2]` `done-2026-05-09 (#26)`
- [x] IEC 60601-2-54:2022 촬영·투시 시험 항목 `[EP:2]` `done-2026-05-09 (#28)`
- [x] IEC 62220-1-1:2015 DQE 측정 절차 `[EP:2]` `done-2026-05-10 (#29)`
- [x] IEC 62304:2006+A1:2015 SW 수명주기 산출물 매핑 `[EP:3]` `done-2026-05-10 (#30)`
- [x] IEC 62133-2 배터리 안전 (Handheld 적용 시) `[EP:1]` `done-2026-05-09 (#27)`

## [P2] 중우선 — 살 붙이기

### 4. QMS
- [x] ISO 13485:2016 절차서 목록 (섹션 4~8)·뼈대 `[EP:2]` `done-2026-05-11 (#31)`
- [x] KGMP vs QMSR vs ISO 13485 차이·통합 전략 (3개 지역 동시 적합) `[EP:3]` `done-2026-05-11 (#32)`
- [x] Design Controls 절차서 템플릿 (7.3) `[EP:2]` `done-2026-05-12 (#33)`
- [x] Document Control / Record Control 절차서 템플릿 `[EP:1]` `done-2026-05-12 (#34)`
- [x] CAPA 절차서 템플릿 `[EP:1]` `done-2026-05-13 (#36)`
- [x] Internal Audit 절차서 템플릿 `[EP:1]` `done-2026-05-13 (#37)`
- [x] Management Review 절차서 템플릿 `[EP:1]` `done-2026-05-13 (#38)`
- [x] Supplier Control 절차서 템플릿 `[EP:1]` `done-2026-05-13 (#41)`

### 5. 사이버보안·AI
- [x] FDA Threat Model 작성 가이드·STRIDE 기반 샘플 `[EP:2]` `done-2026-05-13 (#42)`
- [x] SBOM 생성 실무 (SPDX vs CycloneDX 비교·도구) `[EP:2]` `done-2026-05-14 (#43)`
- [x] PCCP 작성 가이드 (AI-enabled device, 2024-12-03 Final) `[EP:2]` `done-2026-05-15 (#45)`
- [x] MDCG 2019-16 Rev.1 Cybersecurity 대응 체크리스트 `[EP:1]` `done-2026-05-14 (#44)`
- [x] IEC 81001-5-1:2021 수명주기 활동 매핑 `[EP:2]` `done-2026-05-16 (#47)`
- [x] MFDS 사이버보안 가이드라인 (2025.01 개정) 대응 `[EP:2]` `done-2026-05-17 (#49)`

### 6. 라벨·IFU
- [x] IFU 필수 요소 3개 지역 비교표 `[EP:2]` `done-2026-05-18 (#50)`
- [x] ISO 15223-1:2021 심볼 적용 가이드 `[EP:1]` `done-2026-05-15 (#46)`
- [x] UDI 구조 3개 지역 비교 (GUDID / Eudamed / UDI포털) `[EP:1]` `done-2026-05-16 (#48)`
- [x] 디지털의료기기 표시기재 가이드라인 (MFDS, 2025.01) 대응 `[EP:1]` `done-2026-05-18 (#51)`

### 7. 임상
- [x] Clinical Evaluation (MDR) 심화 - 동등성·충분성 기준 `[EP:3]` `done-2026-05-18 (#52)`
- [x] FDA Clinical Data 필요 판단 기준 (510(k) vs De Novo vs PMA) `[EP:2]` `done-2026-05-19 (#53)`
- [x] MFDS 임상시험 계획 승인 절차 `[EP:2]` `done-2026-05-19 (#54)`
- [x] MDCG 2020-5 (Equivalence) / 2020-6 (Legacy) / 2020-7 (PMCF Plan) / 2020-8 (PMCF Report) 요약 `[EP:2]` `done-2026-05-20 (#55)`

## [P3] 실전 대응

### 8. 심사 Q&A
- [x] FDA Additional Information (AI) 대응 전략·샘플 답변 `[EP:2]` `done-2026-05-20 (#57)`
- [x] FDA RTA (Refuse to Accept) 회피 체크리스트 `[EP:1]` `done-2026-05-20 (#56)`
- [x] MFDS 보완자료 대응 전략·샘플 `[EP:2]` `done-2026-05-21 (#58)`
- [x] NB Deficiency Letter 대응 전략·샘플 `[EP:2]` `done-2026-05-21 (#60)`
- [x] 3개 지역 공통 빈번 지적사항 Top 20 `[EP:2]` `done-2026-05-22 (#61)`

### 9. Post-Market
- [x] PMS Plan 템플릿 (MDR Annex III) `[EP:2]` `done-2026-05-22 (#62)`
- [x] PSUR 템플릿 (MDR) `[EP:2]` `done-2026-05-22 (#63)`
- [x] FDA MDR (Medical Device Reporting) 21 CFR 803 절차 `[EP:2]` `done-2026-05-22 (#64)`
- [x] Vigilance 실무 (MDR) - Serious Incident 판정 기준 `[EP:1]` `done-2026-05-22 (#65)`
- [x] FSCA / FSN 절차·템플릿 `[EP:1]` `done-2026-05-22 (#66)`
- [x] MFDS 이상사례·부작용 보고 절차 `[EP:1]` `done-2026-05-22 (#67)`

### 10. 전문가 교육 Week 2~12
- [x] Week 2: 국내 MFDS 체계 상세 (커리큘럼·실습·평가) `[EP:2]` `done-2026-05-23 (#68)`
- [x] Week 3: FDA 510(k) 상세 `[EP:2]` `done-2026-05-23 (#69)`
- [x] Week 4: MDR EU 체계 상세 `[EP:2]` `done-2026-05-24 (#70)`
- [x] Week 5: 국제표준 (ISO 13485 / 14971 / 62366) `[EP:2]` `done-2026-05-24 (#71)`
- [x] Week 6: X-ray 특화 표준 (60601 시리즈·62220) `[EP:2]` `done-2026-05-25 (#72)`
- [x] Week 7: 기술문서 작성 실습 (STED) `[EP:3]` `done-2026-05-25 (#73)`
- [x] Week 8: 사이버보안·AI (SBOM·Threat Model·PCCP) `[EP:2]` `done-2026-05-26 (#74)`
- [x] Week 9: 임상평가 `[EP:2]` `done-2026-06-01 (#77)`
- [x] Week 10: QMS 심사 대응 `[EP:2]` `done-2026-06-01 (#76)`
- [x] Week 11: 심사 Q&A 시뮬레이션 `[EP:2]` `done-2026-06-01 (#79)`
- [x] Week 12: 종합 실습·평가 `[EP:3]` `done-2026-06-01 (#80)`

## [P4] 고도화

- [x] 과제 Gate Review 체크리스트 (DHF → STED → 제출 → AI 대응 → 승인) `[EP:2]` `done-2026-06-02 (#81)`
- [x] 원가·일정 예측 모델 (지역·등급·시험비) `[EP:3]` `done-2026-06-02 (#83)`
- [x] 글로벌 동시 제출 전략 (MDSAP 활용) `[EP:2]` `done-2026-06-02 (#82)`
- [x] 예상 Q&A DB (3개 지역·제품별) `[EP:3]` `done-2026-06-03 (#84)`
- [x] 규제 변경 모니터링 자동화 (법령·가이던스 RSS) `[EP:2]` `done-2026-06-03 (#85)`
- [x] 경쟁 제품 510(k) Summary 분석 DB `[EP:2]` `done-2026-06-04 (#86)`
- [x] EUDAMED 모듈별 등록 실무 가이드 `[EP:2]` `done-2026-06-04 (#87)`

## [P5] 유지관리·정기 패치 (Phase 5 — 2026-06 도입)

> BACKLOG 전체 완료(EP 165/165, 2026-06-04) 이후 정기 규제 변경 패치 시리즈.
> 매 분기 1~2개 항목 처리. EP:3(L) 기준.

- [x] 2026 Q2 규제 업데이트 종합 패치 (MFDS/FDA/MDR) `[EP:3]` `done-2026-06-04 (#88)`
- [x] 기존 KB 문서 갱신 — QMSR 발효·HF 가이던스 반영 (KGMP 비교·IEC 62366 템플릿) `[EP:2]` `done-2026-06-05 (#89)`
- [x] 2026 Q3 규제 업데이트 패치 (MDR 간소화 채택 여부 반영) `[EP:3]` `done-2026-06-05 (quarterly)`
- [x] FDA AI/ML Device Action Plan 2026 업데이트 (PCCP 운영 사례 분석) `[EP:2]` `done-2026-06-05 (quarterly)`
- [x] MFDS 디지털의료제품법 하위 법령 보완 (2026 신규 고시) `[EP:2]`