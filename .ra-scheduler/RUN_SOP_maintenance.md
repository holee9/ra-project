...
- **실행 준비 산출물도 함께 유지**: `착수준비_마스터_로드맵.md`·`착수준비_제품별_산출물체크리스트.md`·`착수준비_진척_트래커.md`. 규제 변경이 착수 산출물에 영향 시 트래커 게이트/기한을 갱신한다.
- **준비-시스템 관점(고정)**: 본 KB는 개발/제조 착수 "준비" 체계다. 제품 실측·firm 사양(EMDN leaf·배터리·시험성적서·진행현안 등)은 **실행단계 입력**이며 KB 결함·검증부채로 취급하지 않는다.
- **검증 베이스라인(재플래그 금지)**: 검증①~⑬(VERIFICATION_REGISTER P1/P2 전건) 및 IEC 62304 표기·고영향 무태그 스폿검증은 2026-06 완료됨. 이미 폐쇄된 항목을 신규 부채로 재등록하지 말 것. 신규 의심만 등록한다.
...
2. 회당 **1~2개** 처리(스코프 가드). 우선순위: (a) 마감 임박 규제, (b) 표준 개정(IEC 62304 Ed.2 등), (c) P5 정기 패치. (d) 무태그 문서 전수 재검증 순차 진행(이슈 #110, 저위험·여력 시).
